...
public class NumberRunningTextView extends AppCompatTextView {
    ...
    public NumberRunningTextView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.baselib_NumberRunningTextView);
        frameNum = ta.getInt(R.styleable.baselib_NumberRunningTextView_baselib_frameNum, 30);
        textType = ta.getInt(R.styleable.baselib_NumberRunningTextView_baselib_textType, MONEY_TYPE);
        useCommaFormat = ta.getBoolean(R.styleable.baselib_NumberRunningTextView_baselib_useCommaFormat, true);
        runWhenChange = ta.getBoolean(R.styleable.baselib_NumberRunningTextView_baselib_runWhenChange,true);
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case MONEY_TYPE://金钱数字的滚动
                    ...
                    if (useCommaFormat) {
                        //使用每三位数字一个逗号的格式
                        String formatStr = com.example.baselibrary.util.NativeUtil.addComma(str);//三位一个逗号格式的字符串
                    ...
                    if (nowMoneyNum < finalMoneyNum) {
                        ...
                        //已经达到目标的金额数字，显示最终的数字
                        if (useCommaFormat) {
                            NumberRunningTextView.this.setText(com.example.baselibrary.util.NativeUtil.addComma(formatter.format(finalMoneyNum)));
                        } else {
