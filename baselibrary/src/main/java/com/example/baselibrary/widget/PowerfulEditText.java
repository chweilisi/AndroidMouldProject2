...
public class PowerfulEditText extends AppCompatEditText {
    ...
    public PowerfulEditText(Context context, AttributeSet attrs, int defStyle) {
        ...
        ta = context.obtainStyledAttributes(attrs, R.styleable.baselib_PowerfulEditText);

        funcType = ta.getInt(R.styleable.baselib_PowerfulEditText_baselib_funcType, TYPE_NORMAL);

        eyeCloseResourseId = ta.getResourceId(R.styleable.baselib_PowerfulEditText_baselib_eyeClose, R.mipmap.baselib_eye_close);
        eyeOpenResourseId = ta.getResourceId(R.styleable.baselib_PowerfulEditText_baselib_eyeOpen, R.mipmap.baselib_eye_open);
    ...
    private void init() {
        ...
        if (mRightDrawable == null) {
            ...
            if (funcType == TYPE_CAN_CLEAR) {
                //有清除功能，设置默认叉号选择器
                mRightDrawable = getResources().getDrawable(R.drawable.baselib_delete_selector);
        ...
        if (leftDrawable != null) {
            leftWidth = ta.getDimensionPixelOffset(R.styleable.baselib_PowerfulEditText_baselib_leftDrawableWidth,leftDrawable.getIntrinsicWidth());
            leftHeight = ta.getDimensionPixelOffset(R.styleable.baselib_PowerfulEditText_baselib_leftDrawableHeight,leftDrawable.getIntrinsicHeight());
            leftDrawable.setBounds(0, 0, leftWidth, leftHeight);
        }

        if (mRightDrawable != null) {
            rightWidth = ta.getDimensionPixelOffset(R.styleable.baselib_PowerfulEditText_baselib_rightDrawableWidth,mRightDrawable.getIntrinsicWidth());
            rightHeight = ta.getDimensionPixelOffset(R.styleable.baselib_PowerfulEditText_baselib_rightDrawableWidth,mRightDrawable.getIntrinsicHeight());
