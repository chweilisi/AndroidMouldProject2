...
public class DownloadApk {
    ...
    public static void downloadApk(Context context, String url, String title, final String appName) {
        //获取存储的下载ID
        long downloadId = SharedPrefUtils.getInstance().getSharedlongData(DownloadManager.EXTRA_DOWNLOAD_ID, -1L);
        if (downloadId != -1) {
            //存在downloadId
    ...
    private static void start(Context context, String url, String title, String appName) {
        if (hasSDKCard()) {
            ...
            SharedPrefUtils.getInstance().setSharedlongData(DownloadManager.EXTRA_DOWNLOAD_ID, id);
        } else {
    ...
    public static void removeFile(Context context) {
        String filePath = SharedPrefUtils.getInstance().getSharedStringData("downloadApk", null);
        if (null != filePath) {
